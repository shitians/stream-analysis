	...
	*/
		...
		***/
		topology.newStream("tweets", spoutTweets)
			.parallelismHint(1)
          		...
          		// .each(new Fields("text","tweetId","user"), new PrintFilter())
	 ...
	 *     https://github.com/fhussonnois/storm-trident-elasticsearch/blob/13bd8203503a81754dc2a421accff216b665a11d/src/main/java/com/github/fhuss/storm/elasticsearch/state/QuerySearchIndexQuery.java 
	    ...
	    topology
            ...
            //.each(new Fields("query","indices","types"), new Print("query field"))
	...
	System.out.println("STARTING DRPC QUERY PROCESSING");
	   ...
	   ***/
	   ...
	   String query3 = QueryBuilders
				.termQuery("text", "apples").buildAsBytes().toUtf8();
	   /*matchQuery*/
 	   String query4 = QueryBuilders
				.matchQuery("text","happy").buildAsBytes().toUtf8();	 
       /*moreLikeThisQuery*/
	   String query5 = QueryBuilders
				.moreLikeThisQuery("text")
	   			.likeText("love").minTermFreq(1)
	   			.maxQueryTerms(10)
				.buildAsBytes().toUtf8();
	   /*fuzzyQuery*/
	   String query6 = QueryBuilders.fuzzyQuery("text","the").buildAsBytes().toUtf8();
 
       String drpcResult;
       for (int i = 0; i < 5; i++) {
          drpcResult = drpc.execute("search_event", query1+" "+index_name+" "+index_type);
          System.out.println("DRPC RESULT: " + drpcResult);
